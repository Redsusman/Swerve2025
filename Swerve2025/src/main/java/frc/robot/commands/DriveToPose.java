...
public class DriveToPose extends Command {
  ...
  public DriveToPose(Supplier<Pose2d> robotPose, Supplier<Pose2d> targetPose) {
  ...
  public void execute() {
    ...
    double xSetpoint = xController.calculate(currentPose.getX(), backupPose.getX());
    double ySetpoint = yController.calculate(currentPose.getY(),backupPose.getY());
    double thetaSetpoint = thetaController.calculate(currentPose.getRotation().getRadians(),
        backupPose.getRotation().getRadians());
    ...
    swerve.setControl(robotSpeeds.withSpeeds(ChassisSpeeds.fromFieldRelativeSpeeds(xSetpoint, ySetpoint,
      ...
      // System.out.println("xError: " + xController.getError() +" yError: " + yController.getError() + " thetaError: " + thetaController.getError());
